...
*/
							...
							let price_obj;
							let arr_price_obj = arr.filter((element) => {//работа с массивом!!!
	...
	// 							console.log(element.name+"("+element.typeObject+")");
							...
							let arr_price = Object.values(price_obj).slice(15,25);//делает объект price_obj массивом и вырезает ценники => массив цен
							console.log("а это массив цен");
				        	...
				        	arr_menu.forEach(function(element,i){
					        	...
					        	divrItem2.className = "col-md-6";
								divrItem2.innerHTML = 
						        
						        	`<div class="card mb-4 shadow-sm">
						            	<img src="${element.picture}" class="bd-placeholder-img card-img-top" width="100%" height="400">
										<div class="card-body">
							                	...
							                	<div class="btn-group">
													<div class="input-group input-group-sm mb-3">
														<div class="input-group-prepend">
							  							...
							  							<input type="text" class="form-control" readonly aria-label="Small" aria-describedby="inputGroup-sizing-sm" size="2" style=" background: white; border: 1px solid #02aeaf; " value="0">
...
obXhr.onreadystatechange =() => {
	...
	let arr_main = JSON.parse(obXhr.response);
	...
	function Zap_main(){
		...
		//Печать первых 8 заведений
		let rate_array_top_8 = arr_main.sort((a, b) => a.rate < b.rate ? 1 : -1);//все сам))))) - это фильтрация массива объектов по рейтингу
		
		for (let i = 0; i < 8; i++){//выводим 8 элементов начиная с конца в отсортированном массиве
        console.log(rate_array_top_8[i]);//выводишь
		...
		divrItem = document.createElement('div');
					...
					divrItem.innerHTML = 
						`<div class="card mb-4 shadow-sm">
							<div class="card-body">
								<div class="row">
									<div class="col-12" style=" height: 60px;"><p class="card-text" id="card">${String(rate_array_top_8[i].name)}(${String(rate_array_top_8[i].typeObject)})</p></div>
									<div class="col-12" style=" height: 120px;"><hr><small class="text-muted">${String(rate_array_top_8[i].address)}</small></div>
								</div>	
								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
									...
									</svg> ${String(rate_array_top_8[i].rate/10)}</small>
		              			</div>
		            		</div>
						</div>`
		...
		work_with_menu(rate_array_top_8);
	...
	document.getElementById("FilterFindButton").onclick = () => {
		...
		let k = 0;
		let arr_print = arr_main.filter((element) => {
	    	if((element.admArea == admAr) || (admAr.length == 0))
				if(Distr == element.district || Distr.length == 0)
					if( typeOb == element.typeObject || typeOb.length == 0)
						if(((socialDisc == "Есть") && (element.socialPrivileges == 1)) || ((socialDisc == "Нет") && (element.socialPrivileges == 0)) || (socialDisc.length == 0))
						{
							k++;
							return element;
    					}
...
// 			  	getCheckbox('gridRadios');
		  		...
		  		pItem.innerHTML = `<big>Итого: ${itog +250}<big>`;
		  		el.append(pItem)
		  	}
  		}
	...
	for (let i = 0; i < CheckBox.length; i++) {
		...
		CheckBox[i].onclick = () =>  {
			...
			if (CheckBox[i].checked) {
				...
				if (CheckBox[i].value == "option5"){
// 					let elements_price = document.getElementsByName('itog');
					for (let el of elements_price) {
						el.innerHTML = "";
		  			}
					itog_options = itog * 1.6;
					for (let el of elements_price) {
						el.innerHTML = "";
						pItem = document.createElement('p');
						pItem.innerHTML = `<big>Итого: ${itog_options+250}</big>`;
						el.append(pItem)
		  			}
					...
					divItem_choosed_options.innerHTML = 
											...
											<div class="col-4 themed-grid-col" name="pos_6" align="left">x2 позиций</div>

											<div class="col-3 themed-grid-col" align="center">+60% </div>
								`;
					document.getElementById('option5_1').append(divItem_choosed_options);
...
// 					let elements_price = document.getElementsByName('itog');
				...
				if (CheckBox[i].value == "option5"){
					document.getElementById('option5_1').innerHTML = "";
					...
					for (let el of elements_price) {
		  			...
		  			if(document.getElementById('disc_30').length != null ){
						document.getElementById('disc_30').innerHTML = "";
						document.getElementById('disc_30').innerHTML = `${itog_options*0.7 +250}`;
					}		  			
